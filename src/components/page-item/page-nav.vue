<template>
  <a-affix :offset-top="0">
    ...
    >
      ...
      <a-input v-model="login.password" placeholder="password" size="large" type="password" class="nav-input"
               @pressEnter="loginCheck">
        <a-icon slot="prefix" type="lock"/>
      </a-input>
      <div style="margin-top: 20px; text-align: center">
        <a-button style="width: 50%" type="primary" @click="loginCheck">登录</a-button>
      </div>
      ...
      <div style="width: 100%; text-align: center">
        <a-button
            ...
            :disabled="ssoLock"
            @click="ssoLogin">
          <img style="width: 20px; height: 20px; overflow: unset" src="favicon.ico" alt=""/>
    ...
    >
      <a-form-model :rules="regRules" :model="reg" ref="regForm">
        ...
        <a-form-model-item prop="password2">
          <a-input v-model="reg.password2" placeholder="Repeat Password *" size="large" type="password"
                   class="nav-input">
            <a-icon slot="prefix" type="lock"/>
          </a-input>
        </a-form-model-item>
        ...
        <a-form-model-item prop="captcha">
          <a-input v-model="reg.captcha" placeholder="Captcha *" size="large" class="nav-input">
            ...
            <img slot="suffix" :src="captchaUrl"
                 @click="() => {this.captchaUrl = this.host+'/captcha/get?p='+new Date().getTime()}" alt="看不清? 单击更换图片"
                 width="100%" title="看不清? 单击更换图片">
          </a-input>
        </a-form-model-item>
      </a-form-model>
    </a-modal>
