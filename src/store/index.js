...
const PermissionLevel = {
    ANYONE: -999, FORBIDDEN: -1, COMMON: 0, DATA_VIEWER: 100, CODE_VIEWER: 200, ADMIN: 999
}

const ContentTypeText = ['UNKNOWN', 'ACM/ICPC', 'OI', 'IOI', 'Codeforces']
...
export default new Vuex.Store({
    state: {
        ...
        loginBySSO(state, data) {
            let that = this._vm
            that.$http.get(ssoapi + '/ticket')
                .then(resp => {
                    let ticket = resp.data.data.ticket
                    let username = resp.data.data.username
                    let sendData = new FormData()
                    sendData.append('username', username)
                    sendData.append('ticket', ticket)
                    that.$http.post(host + '/user/ssoLogin', sendData).then(resp => {
                ...
                .catch(err => {
                    if (err.response) {
                        let callbackUrl = router.resolve({name: 'ssoCallback'}).href
